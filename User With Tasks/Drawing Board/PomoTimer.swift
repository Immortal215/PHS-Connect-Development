...
struct Pomo: View {
    ...
    var body: some View {
                ...
                HStack {
                    ...
                    VStack {
                        ...
                        .onChange(of: textPomo) {
                            if Int(textPomo) ?? 0 != 0 && Int(textPomo) != nil
                                && Int(textPomo) ?? 0 <= 60
                            {
        ...
        .onAppear {
            ...
            endString =
                myTimerPomo?.isValid ?? false
                ? Date(timeIntervalSinceNow: Double(progressTimePomo))
    ...
    UNUserNotificationCenter.current().requestAuthorization(options: [
        ...
        if granted {
            ...
            UNUserNotificationCenter.current().add(request) { error in
                if let error = error {
                    print(
                        "Error adding notification: \(error.localizedDescription)"
                    )
                } else {
...
func pomoResulter(pomoTime: Int, breakTime: Int, breakText: Bool) -> String {
    ...
    return
        "\(breakText ? "Pomo" : "Break") Time : \(breakText ? (pomoTime >= 60 ? "\(pomoTime/60) Minutes (\(pomoTime)s)" : "\(pomoTime) Seconds") : (breakTime >= 60 ? "\(breakTime/60) Minutes (\(breakTime)s)" : "\(breakTime) Seconds"))"
}
