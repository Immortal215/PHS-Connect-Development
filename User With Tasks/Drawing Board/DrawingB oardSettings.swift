...
struct Settinger: View {
    ...
    @State var dueDaters: [String] = [
        "Today", "One Hour", "6 Hours", "One Day", "Two Days", "Four Days",
        "Five Days",
    ]
    ...
    @State var organizationOptions: [String] = [
    ...
    @State var textPomo = ""
    @State var timerOptions: [String] = [
        "Custom", "900", "1200", "1500", "1800",
    ]

    @AppStorage("breakTime") var breakTime = 300
    @State var textBreak = ""
    @State var timerOptionsBreak: [String] = [
        "Custom", "180", "240", "300", "360",
    ]
    ...
    @State var hexSubjectColor: Color = Color(hexadecimal: "#91E2FD")

    @AppStorage("titlecolor") var titleColor: String = "#E2FFC2"
    @State var hexTitleColor: Color = Color(hexadecimal: "#E2FFC2")
    ...
    var body: some View {
                ...
                Form {
                    ...
                    Section("Planner") {
                        LabeledContent {
                            ...
                            Text(
                                "Every new objective will start in **\(dueDateSetter)** at **\(datedVar.formatted(date: .omitted, time: .shortened))**"
                            )
                        }
                        ...
                        Toggle(isOn: $emptyClear) {
                            ...
                            Text(
                                "A button to clear all spaces in your description"
                            )
                        }
                        ...
                        Toggle(isOn: $subjectPicker) {
                            Text("Subject Picker")
                            Text(
                                "A picker that allows you to choose from all subjects"
                            )
                        }
                    ...
                    Section("Pomo Timer") {
                            ...
                            Picker("", selection: $pomoTime) {
                                if timerOptions.contains(String(pomoTime)) {
                                    ...
                                    Section("Custom") {
                                        if pomoTime >= 60 {
                                            Text(
                                                "Custom \(pomoTime / 60) Minutes"
                                            ).tag(pomoTime)
                                        } else {
                                            Text("Custom \(pomoTime) Seconds")
                                                .tag(pomoTime)
                                        }
                                    }
                        ...
                        HStack {
                            ...
                            Picker("", selection: $breakTime) {
                                ...
                                {
                                    ForEach(timerOptionsBreak, id: \.self) {
                                        i in
                                        if i != "Custom" {
                                            if i != "300" {
                                                Text(
                                    ...
                                    Section("Custom") {
                                        if breakTime >= 60 {
                                            Text(
                                                "Custom \(breakTime / 60) Minutes"
                                            ).tag(breakTime / 60)
                                        } else {
                                            Text("Custom \(breakTime) Seconds")
                                                .tag(breakTime / 60)
                                        }
                                    }
                            ...
                            TextField("Custom (Seconds)", text: $textBreak)
                                ...
                                .onChange(of: textBreak) {
                                    ...
                                    if Int(textBreak) ?? 0 != 0
                                        && Int(textBreak) != nil
                                        && Int(textBreak) ?? 0 <= 3600
                                    {
                    ...
                    Section("Home") {
                        HStack {
                            Picker(
                                "Urgent Assignments Count",
                                selection: $urgentCount
                            ) {
                                Text("2").tag(2)
                        ...
                        LabeledContent {
                            ...
                            if drawer {
                                Text(
                                    "Draw Line Width: \(chosenWidth, specifier: "%.1f")"
                                )
                                Button("Reset") {
                                    chosenWidth = 5
                                }
                        ...
                        Picker("Alarm Volume", selection: $alarmLevel) {
                            Text("Base Alarm Level (Low, Wearing Headphones)")
                                .tag("myalarm.mp3")
                            Text("Alarm Level (Medium, Working W/O Headphones)")
                                .tag("myalarmMedium.mp3")
                            Section("Dangerous") {
                                Text("Alarm Level (High, Home Alone)").tag(
                                    "myalarmHigh.mp3"
                                )
                            }
                    ...
                    Section("Misc.") {
                        ...
                        LabeledContent {
                            ...
                            if tabStyle {
                                Text(
                                    "Tab Bar Opacity: \(Int(chosenOpacity * 100))%"
                                )
                                Button("Reset") {
                                    chosenOpacity = 0.8
                                }
                        ...
                        .sheet(isPresented: $clear) {
                            ...
                            ZStack {
                                Button(
                                    clearCheck ? "NO GOING BACK NOW" : "DELETE"
                                ) {
                                    clearCheck.toggle()
                                    if clearCheck == false {
                                        resetDefaults()
        ...
        .onAppear {
            ...
            hexSubjectColor = Color(hexadecimal: subjectColor)
            hexTitleColor = Color(hexadecimal: titleColor)
            hexDescriptionColor = Color(hexadecimal: descriptionColor)
