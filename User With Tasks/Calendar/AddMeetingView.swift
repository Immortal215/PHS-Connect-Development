            ...
            ScrollView {
                ...
                LabeledContent {
                    DatePicker("", selection: $endTime)
                        .onChange(of: endTime) {
                            timeDifference = abs(
                                endTime.distance(to: startTime)
                            )
                        }
                } label: {
                    let isInvalidEndTime =
                        endTime <= startTime || !isSameDay(endTime, startTime)
                    ...
                    .foregroundStyle(
                        isInvalidEndTime || isTooShort ? .red : .primary
                    )
                ...
                LabeledContent {
                    ...
                    .onChange(of: visibleByWho) {
                        if let leaders = leaderClubs.first(where: {
                            $0.clubID == clubId
                        })?.leaders {
                            switch visibleByWho {
                            case "Everyone":
                            ...
                            case "Only Leaders":
                                visibleBy = leaders
                            case "Custom":
                                visibleBy = visibleBy.filter({
                                    !leaders.contains($0)
                                })  // needed so when editing a club with custom people, it doesnt reset visibleBy to empty [].
                            default:
                                visibleBy = []
