...
func addAnnouncement(announcement: Club.Announcements) {
    ...
    do {
        ...
        if let dictionary = try JSONSerialization.jsonObject(
            ...
            reference.child("clubs").child(announcement.clubID).child(
                "lastUpdated"
...
func addPersonSeen(announcement: Club.Announcements, memberEmail: String) {
    ...
    clubRef.child("peopleSeen").observeSingleEvent(of: .value) { snapshot in
        ...
        clubRef.child("peopleSeen").setValue(Array(Set(peopleSeen))) {
            error,
            _ in
            if let error = error {
                print(
...
func addMeeting(meeting: Club.MeetingTime) {
    ...
    clubReference.child("meetingTimes").observeSingleEvent(of: .value) {
        snapshot in
        ...
        do {
                ...
                as? [String: Any]
            {
                ...
                clubReference.child("lastUpdated").setValue(
...
func replaceMeeting(oldMeeting: Club.MeetingTime, newMeeting: Club.MeetingTime)
{
    ...
    oldClubReference.child("meetingTimes").observeSingleEvent(of: .value) {
        snapshot in
        ...
        oldClubReference.child("meetingTimes").setValue(currentMeetings) {
            error,
            _ in
            if let error = error {
    ...
    clubRef.child("members").observeSingleEvent(of: .value) { snapshot in
        ...
        clubRef.child("members").setValue(Array(Set(members))) { error, _ in
            if let error = error {
                ...
                clubRef.child("lastUpdated").setValue(
                    Date().timeIntervalSince1970
                )
...
func addPendingMemberRequest(clubID: String, memberEmail: String) {
    ...
    clubRef.child("pendingMemberRequests").observeSingleEvent(of: .value) {
        snapshot in
        ...
        clubRef.child("pendingMemberRequests").setValue(
...
func removePendingMemberRequest(clubID: String, emailToRemove: String) {
    ...
    clubRef.child("pendingMemberRequests").observeSingleEvent(of: .value) {
        ...
        if let index = pendingRequests.firstIndex(
            ...
            clubRef.child("pendingMemberRequests").setValue(pendingRequests) {
                error,
                _ in
                if let error = error {
                    print(
...
func addLocationCoords(clubID: String, locationCoords: [Double]) {
    ...
    clubRef.child("locationInSchoolCoordinates").observeSingleEvent(of: .value)
    { snapshot in

        clubRef.child("locationInSchoolCoordinates").setValue(locationCoords) {
            error,
            _ in
            if let error = error {
...
func createClubGroupChat(
    ...
    ref.child(chatID).setValue(chatDict) { error, _ in
        if let error = error {
            ...
            clubsRef.child("chatIDs").observeSingleEvent(of: .value) {
                snapshot in
                var chatIDs = snapshot.value as? [String] ?? []
                if !chatIDs.contains(chatID) {
                    chatIDs.append(chatID)
                    clubsRef.child("chatIDs").setValue(chatIDs)
                    clubsRef.child("lastUpdated").setValue(
                        Date().timeIntervalSince1970
                    )
                }
            }
