        ...
        ZStack(alignment: .bottom) {
            ...
            HStack {
                ...
                VStack(alignment: .trailing) {
                    ...
                    if !viewModel.isGuestUser {
                        Button {
                            if userInfo?.favoritedClubs.contains(club.clubID)
                                ?? false
                            {
                                ...
                                ?? false
                            {
                                Image(systemName: "pin.fill")
                                    .foregroundStyle(.red)
                                    .shadow(
                                        color: darkMode ? .red : .clear,
                                        radius: 3
                                    )
                                    .transition(
                                        .asymmetric(
                                            insertion: .movingParts.pop(.red),
                                            removal: .movingParts.vanish(.red)
                                        )
                                    )
                            } else {
                        ...
                        Button(
                            isClubLeaderOrSuperAdmin(
                                ...
                                : club.members.contains(
                                    ...
                                    ? "Member"
                                    : (club.pendingMemberRequests?.contains(
                                        viewModel.userEmail ?? ""
                                    ) ?? false) && club.requestNeeded != nil
                                        ? "Applied"
                            ...
                            if let email = viewModel.userEmail {
                                if let requestNeeded = club.requestNeeded {  // if you need to request to join
                                    ...
                                    {  // if the club and pending members dont have this user
                                        if var cluber = club
                                            .pendingMemberRequests
                                        {  // if the club has a pendingmemberrequests
                                            cluber.insert(email)
                    ...
                    )
            {
                ...
                VStack {
                    ...
                    Spacer()
                    Text(
                        "^[\(notificationCount) New Notifications](inflect:true)"
                    )  // thingy to nake sure the text plurals stay consistent
                    .font(.subheadline)
