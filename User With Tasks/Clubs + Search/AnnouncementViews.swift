...
struct AddAnnouncementSheet: View {
    ...
    @State var announcement: Club.Announcements = Club.Announcements(
        date: "",
        title: "",
        body: "",
        writer: "",
        ...
        VStack(alignment: .trailing) {
            ScrollView {
                ...
                HStack {
                    ...
                    .padding()
                    .background(
                        Capsule().strokeBorder(Color.gray, lineWidth: 1)
                    )
                        ...
                        if let clubName = clubNames[announcement.clubID] {
                            ...
                            .sheet(item: $selectedAnnouncement) { selected in
                                ...
                                .onAppear {
                                    ...
                                    var mutableAnnouncement = selected
                                        .announcement

                                    addPersonSeen(
                                        announcement: mutableAnnouncement,
                                        memberEmail: (viewModel.userEmail ?? "")
                                    )

                                    if let peopleSeen = mutableAnnouncement
                                        .peopleSeen
                                    {
                                        if !peopleSeen.contains(
                                            viewModel.userEmail ?? ""
                                        ) {
                                            mutableAnnouncement.peopleSeen?
                                                .append(
                                                    viewModel.userEmail ?? ""
                                                )
                                        }
...
struct AllAnnouncementsView: View {
    ...
    var body: some View {
        ...
        ForEach(
            announcements.sorted(by: {
                dateFromString($0.value.date) > dateFromString($1.value.date)
            }),
            id: \.key
        ) { (key, announcement) in
            if let clubName = clubNames[announcement.clubID] {
                    ...
                    .onAppear {
                        ...
                        if let peopleSeen = mutableAnnouncement.peopleSeen {
                            if !peopleSeen.contains(viewModel.userEmail ?? "") {
                                mutableAnnouncement.peopleSeen?.append(
                                    viewModel.userEmail ?? ""
                                )
                            }
                        } else {
...
struct SingleAnnouncementView: View {
    ...
    var body: some View {
        ZStack {
            HStack {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text(
                            dateFromString(announcement.date).formatted(
                                date: .abbreviated,
                                time: .shortened
                            )
                        )
        ...
        .overlay {
            if !(announcement.peopleSeen?.contains(viewModel.userEmail ?? "")
                ?? false) && isClubMember
                && dateFromString(announcement.date)
                    > Date().addingTimeInterval(-604800)
                && !fullView!
            {
