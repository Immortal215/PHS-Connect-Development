...
struct CreateClubView: View {
    ...
    var body: some View {
        ...
        var abletoCreate: Bool {
            return
                (clubTitle != "" && clubDesc != "" && clubAbstract != ""
                && location != "" && !leaders.isEmpty)
        }
        ...
        VStack(alignment: .trailing) {
                ...
                if abletoCreate {
                    Button {
                        if clubId == "" {
                            ...
                            let lastDigit = Int(
                                lastClub.clubID.replacingOccurrences(
                                    of: "clubID",
                                    with: ""
                                )
                            )!
            ...
            ScrollView {
                ...
                LabeledContent {
                    ...
                    Text(
                        "Short Description \(clubDesc.isEmpty ? "(Required)" : "")"
                    )
                ...
                LabeledContent {
                    ...
                    Text(
                        "Club Abstract \(clubAbstract.isEmpty ? "\n(Required)" : "")"
                    )
                ...
                DisclosureGroup(isExpanded: $leaderDisclosureExpanded) {
                    ...
                    HStack {
                        TextField(
                            "Add Leader Email(s) (Required)",
                            text: $addLeaderText
                        )
                        .padding()
                        .changeEffect(
                            .shake(rate: .fast),
                            value: leaderTextShake
                        )
                        .onSubmit {
                            addLeaderFunc()
                        }
                        ...
                        if !leaders.isEmpty && !selectedLeaders.isEmpty {
                            Button {
                                leaders.removeAll(where: {
                                    selectedLeaders.contains($0)
                                })
                ...
                LabeledContent {
                    TextField("Schoology Code (Required)", text: $schoology)
                        ...
                        .onAppear {
                            if schoology != "" {
                                clubType =
                                    schoology.contains("Course")
                                    ? "Course" : "Group"
                            }

                            schoology = schoology.replacingOccurrences(
                                of: " (Course)",
                                with: ""
                            ).replacingOccurrences(of: " (Group)", with: "")
                        }

                    if schoology.replacingOccurrences(of: "-", with: "").count
                        > 12
                    {
                    ...
                    Text(
                        "Schoology Code \(schoology.replacingOccurrences(of: "-", with: "").count < 13 ? "(Set to NONE)" : "")"
                    )
                    .foregroundStyle(
                        schoology.replacingOccurrences(of: "-", with: "").count
                            < 13 ? .red : .primary
                    )
                    .bold(
                        schoology.replacingOccurrences(of: "-", with: "").count
                ...
                LabeledContent {
                    ...
                    Text(
                        "Club Location \(location.isEmpty ? "(Required)" : "")"
                    )
                ...
                DisclosureGroup(
                    "Edit Members",
                    isExpanded: $memberDisclosureExpanded
                ) {
                    VStack {
                        HStack {
                            TextField(
                ...
                DisclosureGroup(
                    "Edit Genres",
                    isExpanded: $genreDisclosureExpanded
                ) {
                    ...
                    LabeledContent {
                        ...
                        Button {
                            if !genres.contains(genrePicker)
                                && genrePicker != "" && genres.count < 5
                            {
    ...
    func addMemberFunc() {
        ...
        if (addMemberText.contains("d214.org")
            || addMemberText.contains("gmail.com"))
            && members.contains(addMemberText) == false
        {
