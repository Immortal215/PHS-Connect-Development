...
struct SignInCalendarPreview: View {
    ...
    var displayedDays: [SignInCalendarSampleDay] {
        isCompact
            ? Array(SignInCalendarSampleDay.samples.dropFirst().dropLast())
            : SignInCalendarSampleDay.samples
    }
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 6) {
                ForEach(displayedDays) { day in
                    SignInCalendarDayCell(day: day, isCompact: isCompact)
                }
            }
...
struct SignInCalendarDayCell: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text(day.weekday)
                .font(.system(size: isCompact ? 10 : 8))
                .foregroundStyle(.secondary)

            Text(day.date)
                .font(.system(size: isCompact ? 12 : 10, weight: .bold))
            ...
            if let badge = day.badge {
                Text(badge)
                    .font(.system(size: isCompact ? 9 : 7, weight: .black))
